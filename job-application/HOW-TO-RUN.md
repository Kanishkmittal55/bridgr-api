...
**Params:** Load `references/preferences.yaml` by default. Override with message (e.g. "Target: X", "Phase A only", "Strict no skill gaps: on").
...
3. Preferences: `references/preferences.yaml` — fill with your defaults; set `matching.strict_no_skill_gaps: true` to only advance zero-gap roles to execute/; message overrides apply
4. Memory (optional): `npx @claude-flow/cli memory init`
