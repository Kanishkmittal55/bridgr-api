...
Sites are loaded from radar/config/sites.yaml, with {query_encoded} and {location_encoded}
placeholders replaced from the user's search configuration.

Migrated from ApplyPilot. No SQLite; returns list[dict] for radar ingestion.
"""
...
from radar_service.llm import get_client
...
def _load_location_filter(search_cfg: dict | None = None) -> tuple[list[str], list[str]]:
    ...
    if search_cfg is None:
        search_cfg = load_searches_config()
    return load_location_config(search_cfg)
...
def load_sites() -> list[dict]:
    """Load scraping target sites from radar/config/sites.yaml."""
    sites_cfg = load_sites_config()
    return sites_cfg.get("sites", [])
...
def _run_one_site(name: str, url: str) -> dict:
    ...
    # Headful retry if page content is tiny (only when display is available, e.g. local dev)
    full_html = intel.get("full_html", "")
    ...
    settings = Settings()
    _headless_only = settings.smartextract_headless_only
    _has_display = bool(os.environ.get("DISPLAY"))
    if len(cleaned_check) < 5000 and full_html and not _is_captcha and not _headless_only and _has_display:
        ...
        log.info("Headful done in %.1fs | JSON-LD: %d | API: %d",
                 collect_time, len(intel["json_ld"]), len(intel["api_responses"]))
    elif len(cleaned_check) < 5000 and full_html and not _is_captcha and (_headless_only or not _has_display):
        log.info("Skipping headful retry (smartextract_headless_only=%s, no display)", _headless_only)
    elif _is_captcha:
...
def build_scrape_targets(
    ...
    queries = [q["query"] for q in queries_cfg] if queries_cfg else []
    if query_override:
        queries = [query_override]

    locs = search_cfg.get("locations", [])
    default_location = location_override or (locs[0]["location"] if locs else "London, UK")
    ...
    for site in sites:
        ...
        if site_type == "search" and queries:
            for q in queries:
                ...
                expanded_url = expanded_url.replace("{query_encoded}", quote_plus(q))
                expanded_url = expanded_url.replace("{query}", quote_plus(q))
...
# -- Run all sites (in-memory, no DB) ---------------------------------------


def _process_result_to_jobs(
    r: dict,
    target: dict,
    accept_locs: list[str],
    reject_locs: list[str],
) -> list[dict]:
    """Extract and filter jobs from _run_one_site result. Adds site/strategy to each job."""
    jobs = r.get("jobs", [])
    site_name = target["name"]
    out = []
    for j in jobs:
        if not _location_ok(j.get("location"), accept_locs, reject_locs):
            continue
        url = j.get("url")
        if not url:
            continue
...
def _run_all(
    ...
    """Run smart extract on all targets. Returns list[dict] of jobs (no DB)."""
    results: list[dict] = []
    all_jobs: list[dict] = []

    def _process_result(r: dict, target: dict) -> None:
        all_jobs.extend(_process_result_to_jobs(r, target, accept_locs, reject_locs))
        ...
        with ThreadPoolExecutor(max_workers=min(workers, len(targets))) as pool:
            ...
            for future in as_completed(future_to_target):
                ...
                try:
                    ...
                    log.warning("Skipping %s (%s): %s", target["name"], target.get("url", "?"), e)
                    continue
                results.append(r)
...
def run_smart_extract(params: dict | None = None) -> list[dict]:
    """Run AI-powered smart extraction. Returns list[dict] of jobs for radar ingestion.

    Args:
        params: Override. Keys: query, location, max_results, region, workers, sites.
                If query+location provided, uses single search; else full config crawl.

    Returns:
        List of job dicts with url, title, company, location, salary, description, site, strategy.
    """
    params = params or {}
    search_cfg = load_searches_config()
    ...
    if not sites:
        log.warning("No sites configured. Create radar/config/sites.yaml.")
        return []
    ...
    if not targets:
        log.warning("No scrape targets. Check config/sites.yaml and searches.yaml.")
        return []
...
def run_smart_extract_streaming(params: dict | None = None) -> Iterator[tuple[str, list[dict]]]:
    """Yield (site_name, jobs) per target for streaming ingestion.

    Same setup as run_smart_extract, but yields each target's jobs as soon as
    that target completes. Runs targets sequentially (workers=1) for streaming.
    """
    params = params or {}
    search_cfg = load_searches_config()
    accept_locs, reject_locs = _load_location_filter(search_cfg)
    ...
    log.info("SmartExtract streaming: %d targets", len(targets))

    cancelled_event = params.get("cancelled_event")

    for i, target in enumerate(targets):
        if cancelled_event and cancelled_event.is_set():
            log.info("SmartExtract streaming cancelled")
            return
        label = target["name"]
        if target.get("query"):
            label = f"{target['name']} [{target['query']}]"
        ...
        jobs = _process_result_to_jobs(r, target, accept_locs, reject_locs)

        if jobs:
            yield target["name"], jobs
